...
class MHSA(nn.Module):
    ...
    def forward(self, x):
        ...
        energy = (1-torch.sigmoid(self.gamma))*content_content + torch.sigmoid(self.gamma)*content_position

        attention = self.softmax(energy)
        
        out = torch.matmul(v, attention.permute(0, 1, 3, 2))/ math.sqrt(C // self.heads)
